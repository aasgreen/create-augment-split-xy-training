...
def thermal_noise_sequence(n_imgs):
    ...
    for i in np.arange(0,n_imgs,number_decrosses+number_augments):
        n_defects = random.randint(1,maxDefect)
...
class Texture():
    def __init__(self, defect_number, n_iterations, n_pictures, size):
    ...
    def gen_defects(self):
        ...
        def d_gen(grid,x,y,k,off):
            ...
            return grid
        grid = self.texture
        cluster = self.size[0]/self.defect_number
        for i in range(self.defect_number):
            dxp = random.randint(0,self.size[0]-1)
            dyp = random.randint(0,self.size[0]-1)

            dxn = np.mod(dxp + random.sample([-1,1],1)[0]*np.random.poisson(lam = cluster), self.size[0]-1)
    ...
    def evolve(self):
        ...
        beta = .5
        xy = xyModelFort(self.size[0], 1, 0.0, beta, 0, self.n_pictures, self.n_iterations, 'random', .02, 10)
        xy.setGrid(self.initialState)
        xy.simRun(savegrid=False, savedefect=False, saveHam=False)
        self.texture = np.ascontiguousarray(xy.snapshots['lattice'][-1])
        return xy
