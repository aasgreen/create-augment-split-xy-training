...
class DefectDataset(Dataset):
    ...
    def process_labels(self):
        ...
        df['frame'] = df['names'].str.extract('.*t_(\d*)\.dat').astype('int')
    ...
    def __getitem__(self, idx):
        ...
        sample = [self.to_tensor(img[:,:,0:3]/255).float(), int(label/256**2*1000)]
        return sample
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1 * 4*4, 200)

        self.fc3 = nn.Linear(200, 1000, bias = True)
        ...
        self.dropout_2d = nn.Dropout2d(p=0.4)
    def forward(self, x):
...
data_folder_test = pathlib.Path('.') /'..' / 'data' / 'test_set'
csv_name_test = pathlib.Path('.')/'..'/'data' / 'test_set'
data_folder_train = pathlib.Path('.') /'..' / 'data' /'train_set'
csv_name_train = pathlib.Path('.')/'..'/'data' / 'train_set'


test_dataset = DefectDataset(csv_name_test, data_folder_test)
train_dataset = DefectDataset(csv_name_train, data_folder_train)
...
weight_file = pathlib.Path('./m2.pt')
